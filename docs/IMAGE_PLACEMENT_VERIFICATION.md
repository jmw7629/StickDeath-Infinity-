...
Run35490580529 failed before linking the app: the independent StudioAudioMix target compiles StudioDocument without the command transport. Image placement had introduced a reference to StudioCommandError in that model. The model now reports its own StudioDocumentError; the command executor validates explicit frame/image references and retains its transport-level invalidReference contract. No source list is broadened and no gate is removed. The exact failed AudioMix target, command/image production suites and native SDK/UI compilation must pass on the corrected source before another native run. The failed run has no Simulator app, UI journeys or artifacts; its complete original log and terminal job data are retained.
