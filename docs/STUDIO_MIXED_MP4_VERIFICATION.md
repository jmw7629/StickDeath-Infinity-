...
The following CI run passed all 24 corrected mux groups, 8 mixed-export groups and 17 color-sampling groups. Its Swift compiler then rejected the large mixed-share test closure as too complex to typecheck, before linked compilation or native UI. The same test is now split into named sample-verification and share-lifetime functions with explicit numeric types; all original checks and 18 session groups remain. Local compilation with the CI flags and all 18 groups pass. The next exact head still requires the complete native gate.
