...
The proposed native UI journey draws a real first frame, adds a blank second frame, exports GIF, verifies the two-frame/12-FPS/17-centisecond receipt and decoded thumbnail, opens the native Save to Files share action, cancels, and checks that the file remains available. It must actually execute on the candidate head before claiming native UI verification. Successful destination save/reopen, iPad and physical-device behavior remain unverified. No live upload, charge or test message is sent.
