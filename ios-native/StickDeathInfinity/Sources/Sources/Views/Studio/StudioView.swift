                ...
                ScrollView {
                    VStack(spacing: 8) {
                        ForEach(Array(tips.enumerated()), id: \.offset) { _, tip in
                            HStack(spacing: 14) {
                                Image(systemName: tip.0).font(.body).foregroundStyle(.orange).frame(width: 28)
...
struct FloatingToolbar: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button(action: onPublish) { Image(systemName: "paperplane.fill").toolbarButtonStyle(tint: .orange) }
        }
