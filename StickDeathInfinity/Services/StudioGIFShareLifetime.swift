...
/// At most one GIF can await a UIKit consumer completion in this process.
...
@MainActor
final class StudioGIFShareLifetime: ObservableObject {
    static let shared = StudioGIFShareLifetime()
    ...
    enum ShareError: LocalizedError {
        ...
        var errorDescription: String? {
            "Another GIF is still reserved by the sharing app. Wait for it to finish before sharing again."
        }
    }
    func reserve(_ request: StudioGIFExportSession.ShareRequest) throws -> Lease {
    ...
    private func markPending(_ lease: Lease) {
        ...
        pendingMessage = "Waiting for the sharing app to finish. The GIF is still reserved."
    }
    ...
    @MainActor final class Lease {
        ...
        private weak var owner: StudioGIFShareLifetime?
        private(set) var wasOffered = false
        private(set) var isFinished = false
        fileprivate init(request: StudioGIFExportSession.ShareRequest, owner: StudioGIFShareLifetime) {
