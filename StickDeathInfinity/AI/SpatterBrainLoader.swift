...
///   - Provider-neutral backend boundary via AppConfig.backendURL
...
class SpatterAIEngine {
    ...
    /// Provider-neutral backend URL — when configured, AI requests route through this boundary.
    private let backendEndpoint: URL?

    private var conversationHistory: [ChatMessage] = []

    init() {
        // Backend endpoint configured via AppConfig.shared.backendURL
    ...
    /// Send a message to Spatter AI and get a response.
    /// Routes through provider-neutral backend when configured;
    /// when unavailable, returns a truthful local/knowledge-backed response.
    func chat(userMessage: String) async -> String {
        ...
        // Route through provider-neutral backend when configured
        if let backend = backendEndpoint {
            ...
            guard let jsonData = try? JSONSerialization.data(withJSONObject: body),
                  let url = URL(string: backend) else {
        ...
        // When backend is unavailable, return truthful local/knowledge-backed response
        let localResponse = brain.contextFor(query: userMessage)
        let truncated = String(localResponse.prefix(1000))
        conversationHistory.removeLast()
        return "💡 \(truncated)...\n\n(Spatter AI running on local knowledge — configure AppConfig.backendURL for online AI requests.)"
    }
