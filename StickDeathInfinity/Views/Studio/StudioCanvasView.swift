...
//
// Double ↔ CGFloat Apple boundary:
//   - StrokePoint x/y/pressure are Double (SDCore canonical)
//   - CGPoint/CGSize/CGRect are CGFloat (CoreGraphics)
//   - All conversions are explicit at the boundary
// ═══════════════════════════════════════════════════════════════════════
...
struct StudioCanvasView: View {
    ...
    func drawingGesture(canvasSize: CGSize, geoSize: CGSize) -> some Gesture {
        DragGesture(minimumDistance: 0)
            .onChanged { val in
                ...
                // CGPoint (CGFloat) → StrokePoint (Double) with explicit conversion
    ...
    // Double → CGFloat conversions explicit at every CoreGraphics boundary
    func drawElement(context: inout GraphicsContext, element: DrawnElement, size: CGSize) {
        ...
        case .pencil, .pen, .brush, .marker, .crayon, .eraser, .smudge:
            ...
            // element.width is Double; multiply by CGFloat scale explicitly
        ...
        case .circle:
            ...
            let radiusX = abs(CGFloat(element.points[1].x) - CGFloat(element.points[0].x)) / 2 * scaleX
            let radiusY = abs(CGFloat(element.points[1].y) - CGFloat(element.points[0].y)) / 2 * scaleY
    ...
    func drawLiveStroke(context: inout GraphicsContext, points: [StrokePoint], size: CGSize) {
        ...
        // strokeWidth is Double; convert to CGFloat explicitly
