...
struct AddImagePanel: View {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            ScrollView {
                                LazyVStack(spacing: 8) {
                                    ForEach(vm.mediaAssets) { asset in
                                        MediaAssetRow(asset: asset, vm: vm)
                                    }
                                }
                                .padding(.horizontal, 16)
                            }
        ...
        .fileImporter(isPresented: $showDocumentPicker, allowedContentTypes: [.image]) { result in
            Task {
    ...
    private func handlePhotoImport(_ item: PhotosPickerItem?) async {
        ...
        isImporting = true
        do {
            let asset = try await MediaImportService.shared.importImageFromPhotos(item)
            vm.addMediaAsset(asset)
        } catch {
            print("[AddImage] Photo import error: \(error)")
        }
        isImporting = false
    ...
    private func handleFileImport(_ result: Result<URL, Error>) async {
        ...
        case .success(let url):
            isImporting = true
            do {
                let asset = try await MediaImportService.shared.importImageFromFiles(url)
                vm.addMediaAsset(asset)
            } catch {
                print("[AddImage] File import error: \(error)")
            }
            isImporting = false
...
struct MediaAssetRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button(action: {
                try? MediaImportService.shared.deleteAsset(asset)
                vm.removeMediaAsset(asset.id)
            }) {
...
struct RotoscopeSheet: View {
    ...
    @State private var extractedFrames: [RotoscopeFrame] = []
    @Environment(\.dismiss) var dismiss
    ...
    private var importView: some View {
        ...
        .photosPicker(isPresented: $showVideoPicker, selection: .constant(nil), matching: .videos)
        .fileImporter(isPresented: $showDocumentPicker, allowedContentTypes: [.movie]) { result in
            Task {
                await handleFileImport(result)
            }
        }
    ...
    // MARK: - Reference Controls View
    private var referenceControlsView: some View {
        VStack(spacing: 16) {
            // Video thumbnail
            ...
            VStack(spacing: 8) {
                ...
                Slider(value: $currentTime, in: 0...max(videoDuration, 1))
                    ...
                    .onChange(of: currentTime) { newValue in
                        Task {
                            await extractFrameAtTime(newValue)
                        }
                    }
            }
            .padding(.horizontal, 16)
            ...
            VStack(spacing: 8) {
                HStack {
                    Text("Reference Opacity")
                        ...
                        .foregroundColor(.white.opacity(0.5))
                    Spacer()
                    Text("\(Int((vm.rotoscopeReference?.opacity ?? 0.5) * 100))%")
                        .font(.system(size: 10, weight: .bold, design: .monospaced))
                        .foregroundColor(.red)
                }
                ...
                Slider(
                    value: Binding(
                        get: { vm.rotoscopeReference?.opacity ?? 0.5 },
                        set: { vm.updateRotoscopeOpacity($0) }
                    ),
                    in: 0...1
                )
                .tint(.red)
            ...
            VStack(spacing: 8) {
                Toggle(isOn: Binding(
                    get: { vm.rotoscopeReference?.frameExtractionEnabled ?? false },
                    set: { _ in vm.toggleRotoscopeFrameExtraction() }
                )) {
                    Text("Extract Frames at Project FPS")
                ...
                Toggle(isOn: Binding(
                    get: { vm.rotoscopeReference?.isAboveDrawing ?? true },
                    set: { _ in vm.toggleRotoscopePosition() }
                )) {
                    Text("Show Above Drawing")
                        .font(.system(size: 12, weight: .medium, design: .monospaced))
                        .foregroundColor(.white)
                }
                .tint(.red)
            ...
            // Remove button
            Button(action: {
                vm.rotoscopeVideoAsset = nil
                vm.rotoscopeReference = nil
            }) {
                Text("Remove Reference Video")
                    .font(.system(size: 12, weight: .bold, design: .monospaced))
                    .foregroundColor(.red)
                    .frame(maxWidth: .infinity)
                    .padding()
    ...
    private func handleFileImport(_ result: Result<URL, Error>) async {
        ...
        case .success(let url):
            isImporting = true
            do {
                let asset = try await MediaImportService.shared.importVideoFromFiles(url)
                vm.setRotoscopeVideo(asset)
                await loadVideoMetadata()
            } catch {
                print("[Rotoscope] Import error: \(error)")
            }
            isImporting = false
        case .failure(let error):
            print("[Rotoscope] File import error: \(error)")
        }
    }

    private func loadVideoMetadata() async {
        guard let asset = vm.rotoscopeVideoAsset else { return }

        let avAsset = AVURLAsset(url: asset.localURL)
        videoDuration = try? await avAsset.load(.duration).seconds ?? 0
        ...
        do {
            let (cgImage, _) = try await generator.image(at: CMTime(seconds: 0, preferredTimescale: 600))
            thumbnailImage = UIImage(cgImage: cgImage)
        } catch {
            print("[Rotoscope] Thumbnail error: \(error)")
        }
    }

    private func extractFrameAtTime(_ time: TimeInterval) async {
        guard let asset = vm.rotoscopeVideoAsset else { return }
        ...
        do {
            ...
            print("[Rotoscope] Frame extraction error: \(error)")
        }
    }
}
