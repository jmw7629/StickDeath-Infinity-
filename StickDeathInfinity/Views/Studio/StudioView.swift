...
struct StudioBottomBar: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            // Copy
            BottomBarButton(icon: "doc.on.doc", label: "COPY", enabled: vm.hasSelection || !vm.currentFrame.elements.isEmpty) {
                if vm.hasSelection {
                    vm.copySelected()
                } else {
                    vm.copyAll()
            ...
            // Delete
            BottomBarButton(icon: "trash", label: "DEL", enabled: vm.hasSelection || !vm.currentFrame.elements.isEmpty) {
                vm.deleteSelected()
            }
...
struct BackgroundLibraryPanel: View {
    ...
    let solidColors: [String] = [
        "FFFFFF", "F8F8F8", "E0E0E0", "808080", "404040", "000000",
        "FF0000", "FF6B35", "FFBA08", "22C55E", "0077B6", "7209B7",
    ]
                        ...
                        if selectedCategory == "Solid" {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                                ForEach(solidColors, id: \.self) { hex in
                                    Button(action: {
                                        vm.pushUndoDirect()
                                        vm.frames[vm.currentFrameIndex].backgroundColor = hex
                                        vm.frames[vm.currentFrameIndex].backgroundGradientColors = nil
                                        vm.activePanel = .none
                                    }) {
                                        VStack(spacing: 4) {
                            ...
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                                ForEach(backgrounds, id: \.name) { bg in
                                    Button(action: {
                                        vm.pushUndoDirect()
                                        vm.frames[vm.currentFrameIndex].backgroundGradientColors = bg.colors
                                        vm.frames[vm.currentFrameIndex].backgroundColor = nil
                                        vm.activePanel = .none
                                    }) {
                                        VStack(spacing: 4) {
                                            RoundedRectangle(cornerRadius: 8)
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 16) {
                    ...
                    AddImageOption(icon: "camera.fill", title: "Take Photo", subtitle: "Use camera to capture") {
                        // Camera requires AVCaptureDevice — available in a future release
                    }
                    .disabled(true)
                    .opacity(0.4)
                    ...
                    AddImageOption(icon: "photo.on.rectangle.angled", title: "Photo Library", subtitle: "Choose from your photos") {
                        // Photo picker requires PHPickerViewController — available in a future release
                    }
                    .disabled(true)
                    .opacity(0.4)
                    ...
                    AddImageOption(icon: "folder.fill", title: "Files", subtitle: "Import from Files app") {
                        // File picker requires UIDocumentPickerViewController — available in a future release
                    }
                    .disabled(true)
                    .opacity(0.4)
                    ...
                    AddImageOption(icon: "doc.on.clipboard.fill", title: "Paste from Clipboard", subtitle: "Paste copied image") {
                        // Paste from clipboard — image picking requires PHPickerViewController
                    }
                    .disabled(true)
                    .opacity(0.4)
        ...
        ZStack {
            ...
            VStack(spacing: 16) {
                ...
                HStack(spacing: 12) {
                    ...
                    .disabled(true)
                    .opacity(0.4)
                    .help("Voice preview requires TTS engine — available in a future release")
        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 12) {
                    ...
                    .disabled(true)
                    .opacity(0.4)
                    .help("AI background removal requires Vision ML — available in a future release")
        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 16) {
                    ...
                    .disabled(true)
                    .opacity(0.4)
                    .help("Video recording requires AVFoundation — available in a future release")
                }
